...
shinyApp(
  ui = tagList(
    ...
    dashboardPage(
      skin = "purple",
      dashboardHeader(title = "UoW Bus TimeTable"),
      dashboardSidebar(
        disable = FALSE,
        collapsed = TRUE,
        sidebarMenu(
          menuItem("TimeTable", tabName = "timetable"),
          menuItem("About", tabName = "info"),
          menuItem("Version Log", tabName = "log")
        )
      ),
      dashboardBody(
        tabItems(
                    ...
                    mainPanel(
                      ...
                      fluidRow(
                        selectInput(inputId = "origin",
                                    label = "From: ",
                                    choices = stops,
                                    selected = "Pool Meadow Bus Station",
                                    width = "100%"
                        )
                      ),
                      fluidRow(
                        selectInput(inputId = "dest",
                                    label = "To: ",
                                    choices = stops,
                                    selected = "University of Warwick Bus Interchange",
                                    width = "100%"
                        )
                      ),
                      ...
                      fluidRow(
                          ...
                          div(div(id = "click.12x", valueBoxOutput(outputId = "latest.12x", width = 12)),
                              style="min-width:240px;max-width:100%; float:right"
                              ),
                      ),
                      hr(),
                      fluidRow(
  ...
  server = function(input, output, session){
    ...
    observe({
      ...
      updateSelectInput(session,
                        "dest",
                        label = "To: ",
                        choices = if (x != "University of Warwick Bus Int") c("University of Warwick Bus Interchange"="University of Warwick Bus Int") else stops[!stops %in% c("University of Warwick Bus Int", "Quadrant")],
                        selected = if (x != "University of Warwick Bus Int") c("University of Warwick Bus Interchange"="University of Warwick Bus Int") else c("Pool Meadow Bus Station"="Coventry Pool Meadow")
      )
    })
    
    # Create reactive objects based on origin and dest
    start <- reactive({ input$origin })
